...
const FileUpload: React.FC<FileUploadScreenProps> = ({ navigation, route }) => {
  ...
  const loadCurrentDocument = async () => {
    try {
      if (!user?.id) {
        console.log('No user logged in, skipping document load');
        return;
      }
      
      console.log('Loading current document from AsyncStorage for user:', user.id);
      ...
      if (storedFile) {
        try {
          ...
          console.log('Found document in storage:', parsedFile.name);
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const fileData = {
        ...
        userId: user.id, // Store user ID with file data for verification
      };
